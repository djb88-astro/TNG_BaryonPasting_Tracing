...
class _AdjacentRows(object):
    ...
    def __init__(
        self, treefile, row_start, row_end, row_original=None, filenum=-1, keysel=None
    ):
        # Public attributes
...
class TreeDB:
    ...
    def _get_offset_file(self, snapnum):
        ...
        if snapnum not in self._offset_files.keys():
            self._offset_files[snapnum] = h5py.File(
                "%s/offsets/offsets_%s.hdf5" % (self._treedir, str(snapnum).zfill(3)),
                "r",
            )
    ...
    def get_main_branch(self, snapnum, subfind_id, keysel=None):
        ...
        if rownum == -1:
            print(
                "Subhalo not found: snapnum = %d, subfind_id = %d."
                % (snapnum, subfind_id)
            )
            print(
                "This object probably has zero DM or baryonic (stars + SF gas) elements."
            )
            return None
    ...
    def _get_subhalos_between_root_and_given(self, snapnum, subfind_id, keysel=None):
        ...
        # Create branch instance
        branch = _AdjacentRows(
            treefile, row_start, row_end, row_original=row_end, keysel=keysel
        )
        return branch
    ...
    def get_future_branch(self, snapnum, subfind_id, **kwargs):
        ...
        subtree = self._get_subhalos_between_root_and_given(
            snapnum, subfind_id, **kwargs
        )
